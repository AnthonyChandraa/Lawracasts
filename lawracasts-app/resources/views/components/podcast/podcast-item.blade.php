<a href="{{ route('index_play', $podcast->id) }}">
    <div class="p-4 bg-gray-900 hover:bg-gray-600 flex gap-5 rounded-lg {{ isset($selected) && $podcast->id == $selected->id
        ? "
                    border-2"
        : '' }}"
        style="width: 700px;">
        ...
        <div>
            ...
            <p class="text-md text-gray-300 font-semibold">
                {{ strlen($podcast->description) > 70 ? substr($podcast->description, 0, 70) . '...' : $podcast->description }}
            </p>
        </div>
    </div>
</a>
<div class="flex  mb-2">
    @if (
        ...
        <button @click="toggleEditPodcast()" class="w-1/2 z-20">
            <div class="p-2 border-yellow-600  hover:bg-yellow-400 border-solid border rounded-l-lg mt-3">
                <p class="text-yellow-700 text-sm font-semibold">Edit</p>
            </div>
        </button>
        {{--    </form> --}}
        <form action="{{ route('delete_podcast') }}" method="post" class="w-1/2 mt-3">
            @csrf
            <input type="hidden" name="id" value="{{ $podcast->id }}">
