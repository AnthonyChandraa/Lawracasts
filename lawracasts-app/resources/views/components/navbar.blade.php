        ...
        <div class="relative flex items-center justify-between h-16">
                ...
                <div class="hidden lg:block lg:ml-6">
                    <div class="flex space-x-4">
                        ...
                        <a href="{{ route('index_podcast') }}"
                            class="{{ \Illuminate\Support\Facades\Route::getCurrentRoute()->getName() == 'index_podcast'
                                ? 'bg-gray-900 text-white'
                                : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}  px-3 py-2 rounded-md text-sm
                           font-medium">Podcasts</a>
                        <a href="{{ route('index_course') }}"
                            class="{{ \Illuminate\Support\Facades\Route::getCurrentRoute()->getName() == 'index_course'
                                ? 'bg-gray-900 text-white'
                                : 'text-gray-300 hover:bg-gray-700 hover:text-white' }}  px-3 py-2 rounded-md text-sm
            ...
            <div class="hidden lg:block lg:ml-4">
                <div x-data="{ open: false }" class="flex items-center">
                    ...
                    @else
                        <div class="">
                            <p class="text-white font-bold text-sm">Hi,
                        ...
                        <div class="ml-4 relative flex-shrink-0">
                            <div>
                                <button @click="open = ! open" type="button"
                                    class="bg-gray-800 rounded-full flex text-sm
                             ring-1 ring-gray-500 text-white
                            focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-gray-800 focus:ring-white"
                                    id="user-menu-button" aria-expanded="false" aria-haspopup="true">
                                    <span class="sr-only">Open user menu</span>
                                    <img class="h-8 w-8 rounded-full"
                                        src="{{ \Illuminate\Support\Facades\Auth::check()
                                            ? asset('storage/' . \Illuminate\Support\Facades\Auth::user()->image_url)
                                            : asset('storage/assets/profile.gif') }}"
                                        alt="Profile Picture">
                                </button>
                            </div>
                        ...
                        ring-1 ring-black ring-opacity-5 focus:outline-none"
                                ...
                                <a href="{{ route('index_profile', \Illuminate\Support\Facades\Auth::user()->id) }}"
                                    class="{{ \Illuminate\Support\Facades\Route::getCurrentRoute()->getName() == 'index_profile' ? 'bg-gray-100' : '' }} block px-4 py-2 text-sm text-gray-700"
