<div class="" style="width: 800px">
    ...
    <div class="w-full">
        @foreach ($forums as $f)
            <x-forum.forum-item title="{{ $f->title }}" content="{{ $f->content }}"
                name="{{ $f->user->first_name }} {{ $f->user->last_name }}" count="{{ $f->count }}"
                topicname="{{ $f->topic->name }}" path="{{ route('index_detail', $f->id) }}"
